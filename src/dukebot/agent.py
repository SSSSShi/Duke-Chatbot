...
def create_duke_agent():
    ...
    """
    print("create_duke_agent: Starting...")
    # Get API keys from environment variables
    serpapi_api_key = os.getenv("SERPAPI_API_KEY")

    # Check if API keys are available
    if not serpapi_api_key:
        print("create_duke_agent: ERROR - SERPAPI_API_KEY not found.")
        raise ValueError("SERPAPI_API_KEY not found in environment variables")
    
    print("create_duke_agent: API keys loaded.")
    # Define the tools
    tools = [
        Tool(
    ...
    print("create_duke_agent: Initializing LLM with Bedrock...")
    # Initialize the LLM with Bedrock
    llm = BedrockChat(
        model_id="anthropic.claude-3-sonnet-20240229-v1:0",
        model_kwargs={"temperature": 0.0},
    )
    print("create_duke_agent: LLM initialized.")
    ...
    print("create_duke_agent: Initializing agent...")
    ...
    print("create_duke_agent: Agent initialized and returned.")
    return agent
...
def process_user_query(query):
    ...
    """
    try:
        print("process_user_query: Starting...")
        # Create the agent
        duke_agent = create_duke_agent()
        
        print("process_user_query: Invoking agent...")
        print(f"process_user_query: Query: {query}")
        
        # Process the query using invoke
        print("process_user_query: About to call duke_agent.invoke...")
        response = duke_agent.invoke({"input": query})
        print("process_user_query: Agent invocation complete.")
        print(f"process_user_query: Response type: {type(response)}")
        print(f"process_user_query: Response keys: {response.keys() if hasattr(response, 'keys') else 'No keys'}")
        
        # Extract the agent's response
        result = response.get("output", "I couldn't process your request at this time.")
        print(f"process_user_query: Final result: {result[:100]}...")  # First 100 chars
        return result
